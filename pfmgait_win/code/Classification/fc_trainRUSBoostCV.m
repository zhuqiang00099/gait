function [model, accCV] = fc_trainRUSBoostCV(samples, labels, conf, cvpars)
...
for fix = 1:cvpars.nfolds
   ...
   % Training
   [model_, acc_, acc_test_pc_] = fc_trainRUSBoost(samplesF, labelsF, conf);
   
   % Test current model
   [estimClass, svmscores, acc_test, acc_test_pc] = fc_classifyRUSBoost(samplesFTest, labelsFTest, model_);
   accCV(fix) = acc_test;
...
if cvpars.finalTrain
   [model, acc_, acc_test_pc_] = fc_trainRUSBoost(samples, labels, conf);
else
   model = bestmodel;
end
