function totalFeatures = mj_loadFeatPartitions(featuresPath, partitions, cams, trajectories)
    ...
    for j=1:size(cams, 1)
        for k=1:size(trajectories, 1)
            if partitions(i).mirror
                ...
                pattern = sprintf('*%s_*%s_W%02d_H%02d_M.mat', strtrim(trajectories(k, :)), cams(j, :), partitions(i).nHorizontalPartitions, partitions(i).nVerticalPartitions); 
            else
                ...
                pattern = sprintf('*%s_*%s_W%02d_H%02d.mat', strtrim(trajectories(k, :)), cams(j, :), partitions(i).nHorizontalPartitions, partitions(i).nVerticalPartitions);
            end
