function totalFeatures = mj_loadFeatPartitionsTUM(featuresPath, partitions, trajectories, sequences)
    ...
    for k=1:size(trajectories, 1)
        for l=1:size(sequences, 1)
            if partitions(i).mirror
                ...
                pattern = sprintf('*%s-%s_W%02d_H%02d_M.mat', strtrim(trajectories(k, :)), sequences(l, :), partitions(i).nHorizontalPartitions, partitions(i).nVerticalPartitions);
            else
                ...
                pattern = sprintf('*%s-%s_W%02d_H%02d.mat', strtrim(trajectories(k, :)), sequences(l, :), partitions(i).nHorizontalPartitions, partitions(i).nVerticalPartitions);
            end
